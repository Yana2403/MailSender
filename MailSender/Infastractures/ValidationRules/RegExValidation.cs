...
namespace MailSender.Infastractures.ValidationRules
{
    public class RegExValidation : ValidationRule
    {
        ...
        public override ValidationResult Validate(object value, CultureInfo culture)
        {
            ...
            if (_Regex is null) return ValidationResult.ValidResult; //проверка наличия регулярного выражения

            if (!(value is string str))
                str = value.ToString();

            return _Regex.IsMatch(str)  //если выполняется для строки
                ? ValidationResult.ValidResult
